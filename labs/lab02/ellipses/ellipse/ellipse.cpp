...
Ellipse Ellipse::fit(const std::vector<Point> &p)
{
   ...
   Mat tmp = (Mat_<double>(3, 3) << a, b, d, b, c, f, d, f, g);
   double Delta = determinant(tmp);
   double J = a * c - b * b;
   double I = a + c;

   if (Delta == 0 || J <= 0 || Delta / I >= 0)
      return Ellipse::invalid(); // This is not an ellipse, but another conic
                                 // curve
   else
...
Ellipse::Ellipse(double a, double b, double c, double d, double f, double g)
{
   ...
   size.width = 2 * sqrt((2 * (a * f * f + c * d * d + g * b * b -
                               2 * b * d * f - a * c * g)) /
                         ((b * b - a * c) *
                          (+sqrt((a - c) * (a - c) + 4 * b * b) - (a + c))));
   size.height = 2 * sqrt((2 * (a * f * f + c * d * d + g * b * b -
                                2 * b * d * f - a * c * g)) /
                          ((b * b - a * c) *
                           (-sqrt((a - c) * (a - c) + 4 * b * b) - (a + c))));
   ...
   double temp_c = sqrt(size.width / 2 * size.width / 2 -
...
Point Ellipse::pointAtAngle(double a, Vec2f *normal = 0) const
{
   ...
   if (normal) {
      Vec2f &n = *normal;
      n = Vec2f(+cos(a / 180.0 * M_PI) * size.height / 2.0,
                +sin(a / 180.0 * M_PI) * size.width / 2.0);
      n = rot * n;
   }
...
void Ellipse::calcWeight(const Mat1b &bw, Mat3b *dbg,
   ...
   for (int a = 0; a < nWeightPoints; a++) {
      ...
      if (roi.contains(p)) {
         ...
         if (dbg) {
            ...
            // line(*dbg, p, p+Point(20*cos(tangentAng), 20*sin(tangentAng)),
            // CV_RGB(255, 255, 0));
            if (pOnEllipse)
