...
{
   return !e.isInvalid() && e.size.width >= param.min_width &&
   ...
   while (open.size() > 0) {
      ...
      for (int yy = -1; yy <= +1; yy++) {
         for (int xx = -1; xx <= +1; xx++) {
            ...
            if (pp.x < 0 || pp.x >= bw.size().width || pp.y < 0 ||
                pp.y >= bw.size().height)
               continue;
            if (bw(pp))
               open.push_back(pp);
...
static Ellipse findEllipseRansac(const Mat1b &bw, std::vector<Point> &points,
                                 const ImgProcParameters &param)
{
...
static void findEllipses(const Mat1b &bw, std::vector<Ellipse> &ellipses_out,
                         Mat3b *vis, const ImgProcParameters &param)
{
   int w = bw.size().width;
   ...
   for (auto p : comp) {
      ...
      try {
         Scalar color =
             isNiceEllipse(e, param) ? CV_RGB(0, 255, 0) : CV_RGB(0, 0, 255);
         ellipse(vis, e, color);
      } catch (cv::Exception) {
      }
...
void processImage(Mat &image, MatPhases &phases, std::vector<Ellipse> &ellipses,
                  const ImgProcParameters &param)
{
   Mat image_small;
   ...
   if (s.width > s.height)
      scale =
          std::min((double)fit.width / s.width, (double)fit.height / s.height);
   else
      scale =
          std::min((double)fit.height / s.width, (double)fit.width / s.height);
   s.width *= scale;
   s.height *= scale;
...
Mat debugOnPixelEllipse(Mat1b &bw, Point p, const ImgProcParameters &param,
                        Ellipse &e)
{
   Mat1b tmp;
   Mat3b vis;
